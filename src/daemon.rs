...
/// Create local .desktop overrides to force PDF viewers (Evince/Okular) to run under XWayland on Wayland sessions.
fn setup_wayland_pdf_compatibility() {
    if std::env::var("XDG_SESSION_TYPE").as_deref() == Ok("wayland") {
        let home = match std::env::var("HOME") {
            Ok(h) => std::path::PathBuf::from(h),
            Err(_) => {
                log::debug!("HOME environment variable not set. Skipping Wayland overrides.");
                return;
            }
        };
        ...
        if system_evince.exists() && !local_evince.exists() {
            log::info!("Wayland detected. Copying Evince desktop entry for XWayland override.");
            if let Ok(content) = std::fs::read_to_string(&system_evince) {
                let updated = content.replace("Exec=evince", "Exec=env GDK_BACKEND=x11 evince");
        ...
        if system_okular.exists() && !local_okular.exists() {
            log::info!("Wayland detected. Copying Okular desktop entry for XWayland override.");
            if let Ok(content) = std::fs::read_to_string(&system_okular) {
                let updated = content.replace("Exec=okular", "Exec=env QT_QPA_PLATFORM=xcb okular");
